...
import { Search, Eye, Check, CheckCheck, Ban, UserX, ChevronLeft, ChevronRight, RotateCcw, CalendarX2 } from 'lucide-react'
...
export function AdminAppointments() {
  ...
  // Debounce search input -> search filter
  useEffect(() => {
    const t = setTimeout(() => {
  ...
  }, [search, status, date, barberId, page])
  ...
  const applyStatusUpdate = async () => {
    ...
    try {
      const updated = await updateAppointmentStatus(pendingAction.appointment.id, pendingAction.status)
  ...
  const askConfirm = (appt: Appointment, status: AppointmentStatus, label: string, danger?: boolean) => {
    setPendingAction({
      appointment: appt,
      status,
      title: `${label} appointment?`,
      message: `This will mark booking ${appt.booking_reference} as "${status.replace('_', ' ')}". This action updates the record immediately.`,
      confirmLabel: label,
      danger,
    })
  }

  const hasFilters = useMemo(() => search || status || date || barberId, [search, status, date, barberId])
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        <div>
          <h2 className="text-lg font-semibold text-[var(--admin-text)]">Appointments</h2>
          <p className="mt-1 text-sm text-[var(--admin-text-muted)]">
            {meta ? `${meta.total} total appointment${meta.total === 1 ? '' : 's'}` : 'Manage bookings across the shop.'}
          </p>
        </div>
      </div>
                ...
                <tr>
                  <td colSpan={9} className="px-4 py-14">
                    <div className="flex flex-col items-center gap-3 text-center">
                      <CalendarX2 className="h-8 w-8 text-[var(--admin-text-subtle)]" />
                      <p className="text-sm text-[var(--admin-text-muted)]">
                        {hasFilters ? 'No appointments match these filters.' : 'No appointments yet.'}
                      </p>
                      {hasFilters && (
                        <button
                          type="button"
                ...
                appointments.map((appt) => {
                  ...
                  return (
                    <tr
                      ...
                      className={
                        isToday
                          ? 'cursor-pointer border-l-2 border-l-[var(--admin-accent)] bg-[var(--admin-accent-soft)]/30 hover:bg-[var(--admin-accent-soft)]/50'
                          : 'cursor-pointer hover:bg-[var(--admin-surface-alt)]/60'
                      }
                    >
                      ...
                      <td className="px-4 py-3 text-[var(--admin-text)]">
                        <div className="flex items-center gap-2.5">
                          <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[var(--admin-accent-soft)] text-[11px] font-semibold text-[var(--admin-accent)]">
                      ...
                      <td className="px-4 py-3 text-[var(--admin-text)]">{appt.barber?.name ?? '—'}</td>
                      <td className="px-4 py-3 text-[var(--admin-text-muted)]">
                        <span className="flex items-center gap-2">
                          {formatDate(appt.appointment_date)}
                          {isToday && (
                            <span className="rounded-full bg-[var(--admin-accent)] px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wide text-black">
                              Today
                            </span>
                          )}
                        </span>
                      ...
                      <td className="px-4 py-3 text-[var(--admin-text)]">
                        {appt.service ? formatCurrency(appt.service.price) : '—'}
                      </td>
      ...
      {/* Details modal */}
      {viewing && (
        <Modal title={`Booking ${viewing.booking_reference}`} onClose={() => setViewing(null)} maxWidth="max-w-lg">
          <div className="space-y-3 text-sm">
            ...
            <Row
              label="Payment"
              value={viewing.payment ? `${formatCurrency(viewing.payment.amount)} · ${viewing.payment.method} · ${viewing.payment.status}` : 'No payment recorded'}
            />
